...
- [~] **#22 P6.1** In-repo half DONE: the in-repo fixtures (`test-fixtures/aif-flows/*`, `lib/flows/__tests__/_fixtures/*`, `lib/agents/__tests__/_fixtures/*`) already parse under the unified config + judge-model removal (full unit 5161 green; zero `judge.settings.model`); the multi-session COMPILE path is covered by the topology `sessionGraph` test. **OWNER-GATED remainder**: the cross-repo `maister-plugins` (`/Users/kaa/Work/Projects/Personal/maister-plugins`, SEPARATE repo) engine-2.0.0 manifest rewrite + the multi-session/consensus EXAMPLE flows for #23 — both need the release-order decision (engine-first vs paired) and live-agent validation. **Cross-repo**: maister-plugins is a separate repo (`/Users/kaa/Work/Projects/Personal/maister-plugins`) on its own branch/tag — engine-2.0.0 + `compat.engine_min: 2.0.0` manifests only run against THIS branch's engine, so land the two as a coordinated pair (decide the release order — see residual question). ALSO bump the in-repo fixtures that declare `runner_profiles` (`web/test-fixtures/aif-flows/*`, `web/lib/flows/__tests__/_fixtures/*`, `web/lib/agents/__tests__/_fixtures/*`) to the unified config + judge-model removal so #23 stays green (the SCHEMA change is the breakage risk, not the engine-floor bump — `2.0.0 ≥ 1.1.0` still satisfies a low `engine_min`).
...
### Phase 7 — Close-out
- [x] **#24 P7.1** As-built status flip + ADR/migration renumber check vs main + full verification gate. Migration 0082 DROP `da62de97`; docs flip + renumber `a58bd2e1` (ADR-114 + migr 0080-0082 free vs main). **Final gate GREEN**: web tsc 0 / unit 5161 / eslint 0 (106 files) / db:generate clean / validate:docs:all (306 mermaid + 594 ADR); supervisor tsc 0 / unit 81; supervisor.openapi valid (web.openapi's 2 errors = pre-existing StudioAssistant/ADR-110).
